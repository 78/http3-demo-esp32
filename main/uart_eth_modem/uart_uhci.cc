...
esp_err_t UartUhci::InitGdma(const Config& config) {
    ...
    // Calculate number of DMA nodes needed for TX
...
esp_err_t UartUhci::StartReceive() {
    ...
    size_t mounted = 0;
    gdma_buffer_mount_config_t mount_configs[rx_pool_size_];
    ...
    gdma_link_mount_buffers(rx_dma_link_, 0, mount_configs, mounted, nullptr);
